...
func TestNeo4jServiceRepository_GetServiceById_Success(t *testing.T) {
	...
	if err != nil { t.Fatalf("CreateService error: %v", err) }

	// Act
	svc, err := repo.GetServiceById(ctx, createdID)
	if err != nil { t.Fatalf("GetServiceById error: %v", err) }

	// Assert
	if svc.Id != createdID { t.Fatalf("expected id %s, got %s", createdID, svc.Id) }
...
func TestNeo4jServiceRepository_GetServiceById_NotFound(t *testing.T) {
	...
	if err != nil { t.Fatalf("GetServiceById returned error: %v", err) }
