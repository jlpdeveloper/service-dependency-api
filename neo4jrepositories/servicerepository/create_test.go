...
func TestNeo4jServiceRepository_CreateService_Success(t *testing.T) {
	...
	if err != nil { t.Fatalf("CreateService returned error: %v", err) }
	if id == "" { t.Fatalf("expected non-empty id from CreateService") }
	...
	if err != nil { t.Fatalf("failed to verify created service: %v", err) }
	rec, err := res.Single(ctx)
	if err != nil { t.Fatalf("expected single record verifying service: %v", err) }
	if name, _ := rec.Get("name"); name != input.Name { t.Fatalf("expected name %q, got %q", input.Name, name) }
	if typ, _ := rec.Get("type"); typ != input.ServiceType { t.Fatalf("expected type %q, got %q", input.ServiceType, typ) }
	if desc, _ := rec.Get("description"); desc != input.Description { t.Fatalf("expected description %q, got %q", input.Description, desc) }
	if url, _ := rec.Get("url"); url != input.Url { t.Fatalf("expected url %q, got %q", input.Url, url) }
	if created, _ := rec.Get("created"); created == nil { t.Fatalf("expected non-nil created, got %#v", created) }
}
