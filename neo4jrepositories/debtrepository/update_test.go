...
func TestNeo4jDebtRepository_UpdateStatus_Success(t *testing.T) {
	...
	); err != nil { t.Fatalf("failed to create service node: %v", err) }

	deb := repositories.Debt{Type: "debt", Title: "Needs Update", Description: "status change", ServiceId: serviceID}
	if err := repo.CreateDebtItem(ctx, deb); err != nil { t.Fatalf("CreateDebtItem error: %v", err) }
	...
	if err != nil { t.Fatalf("failed to fetch created debt: %v", err) }
	...
	if err := repo.UpdateStatus(ctx, debtID, newStatus); err != nil { t.Fatalf("UpdateStatus returned error: %v", err) }
	...
	if err != nil { t.Fatalf("failed to verify updated status: %v", err) }
	rec2, err := check.Single(ctx)
	if err != nil { t.Fatalf("expected single record verifying status: %v", err) }
	st, _ := rec2.Get("status")
	if st != newStatus { t.Fatalf("expected status %q, got %q", newStatus, st) }
}
...
func TestNeo4jDebtRepository_UpdateStatus_NotFound(t *testing.T) {
	...
	if err == nil { t.Fatalf("expected error when debt does not exist") }
