...
func TestNeo4jReleaseRepository_GetReleasesByServiceId_SuccessAndPagination(t *testing.T) {
	...
	if err != nil { t.Fatalf("GetReleasesByServiceId page1 error: %v", err) }
	if len(page1) != 2 { t.Fatalf("expected 2 releases on page1, got %d", len(page1)) }
	if !page1[0].ReleaseDate.Equal(releases[0].ReleaseDate) { t.Fatalf("expected first to be %v, got %v", releases[0].ReleaseDate, page1[0].ReleaseDate) }
	if page1[0].Url != "https://notes/2" || page1[0].Version != "2.0.0" { t.Fatalf("unexpected fields for first: %+v", page1[0]) }
	if !page1[1].ReleaseDate.Equal(releases[1].ReleaseDate) { t.Fatalf("expected second to be %v, got %v", releases[1].ReleaseDate, page1[1].ReleaseDate) }
	// optional fields mapping
	if page1[1].Url != "" || page1[1].Version != "1.1.0" { t.Fatalf("unexpected fields mapping on second: %+v", page1[1]) }

	// Act: page 2 size 2 should return the last release (Dec 31)
	page2, err := repo.GetReleasesByServiceId(ctx, serviceID, 2, 2)
	if err != nil { t.Fatalf("GetReleasesByServiceId page2 error: %v", err) }
	if len(page2) != 1 { t.Fatalf("expected 1 release on page2, got %d", len(page2)) }
	if !page2[0].ReleaseDate.Equal(releases[2].ReleaseDate) { t.Fatalf("expected page2 item to be %v, got %v", releases[2].ReleaseDate, page2[0].ReleaseDate) }
	if page2[0].Url != "https://notes/1" || page2[0].Version != "" { t.Fatalf("unexpected fields on page2 item: %+v", page2[0]) }
}
...
func TestNeo4jReleaseRepository_GetReleasesByServiceId_NotFound(t *testing.T) {
	...
	if err == nil { t.Fatalf("expected error for non-existent service") }
...
func TestNeo4jReleaseRepository_GetReleasesByServiceId_BadParams(t *testing.T) {
	...
	if err == nil { t.Fatalf("expected error for page <= 0") }
	var httpErr *customerrors.HTTPError
	if !errors.As(err, &httpErr) { t.Fatalf("expected *customerrors.HTTPError, got %T: %v", err, err) }
	if httpErr.Status != 400 { t.Fatalf("expected HTTP 400, got %d", httpErr.Status) }

	// pageSize <= 0
	_, err = repo.GetReleasesByServiceId(ctx, "some-id", 1, 0)
	if err == nil { t.Fatalf("expected error for pageSize <= 0") }
	if !errors.As(err, &httpErr) { t.Fatalf("expected *customerrors.HTTPError, got %T: %v", err, err) }
	if httpErr.Status != 400 { t.Fatalf("expected HTTP 400, got %d", httpErr.Status) }
}
