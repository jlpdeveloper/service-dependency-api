...
func getServicesByTeam(_ context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
	...
	return &mcp.GetPromptResult{
		...
		Messages: []*mcp.PromptMessage{
			{
				Role: "user",
				Content: &mcp.TextContent{Text: fmt.Sprintf("To get a list of services for a team, use the resource Use the resource: servicemap://teams/%s/services` or the `get_services_by_team` tool, passing in a required 'teamId' parameter."+
